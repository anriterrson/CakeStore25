...
function updateAuthLink() {
    // ამოწმებს თუ იყო შესული ექუნთზე
    ...
    if (!authLinkElement || !authTextElement) {
        return;
    }

    if (isLoggedIn) {
        //USER შევიდა
        authLinkElement.href = "index.html"; 
        
        //profile image
        userIcon.style.display = 'none';
        profileImg.style.display = 'inline-block';
        const username = sessionStorage.getItem('username') || 'Profile';
        authTextElement.textContent = username;

    } else {
    
        authLinkElement.href = "login.html";
    
        userIcon.style.display = 'inline-block';
        profileImg.style.display = 'none';
        
        authTextElement.textContent = "Login";
    }
}
